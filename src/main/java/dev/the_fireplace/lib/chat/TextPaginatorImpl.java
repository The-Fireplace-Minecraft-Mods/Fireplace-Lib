...
public final class TextPaginatorImpl implements TextPaginator {
    ...
    public void sendPaginatedChat(ServerCommandSource targetCommandSource, String switchPageCommand, List<? extends Text> allItems, int pageIndex) {
        CommandOutput messageTarget = targetCommandSource.getEntity() != null ? targetCommandSource.getEntity() : targetCommandSource.getServer();
    ...
    private Text getNextButton(CommandOutput target, String switchPageCommand, int currentPage, int totalPageCount) {
        return currentPage < totalPageCount ? translator.getTextForTarget(target, "fireplacelib.chat.page.next").setStyle(Style.EMPTY.withClickEvent(new ClickEvent(Action.RUN_COMMAND, String.format(switchPageCommand, currentPage +1)))) : new LiteralText("-----");
    }

    private Text getPreviousButton(CommandOutput target, String switchPageCommand, int currentPage) {
        return currentPage > 1 ? translator.getTextForTarget(target, "fireplacelib.chat.page.prev").setStyle(Style.EMPTY.withClickEvent(new ClickEvent(Action.RUN_COMMAND, String.format(switchPageCommand, currentPage -1)))) : new LiteralText("------");
    }
}
